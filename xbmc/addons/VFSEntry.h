...
namespace ADDON
{
  typedef CAddonDll<DllVFSEntry,
                    VFSEntry, VFS_PROPS> VFSEntryDll; //!< Convenience typedef.
  ...
  public:
    ...
    //! \brief Construct from cpluff extension.
    CVFSEntry(const cp_extension_t *ext);

    //! \brief Empty destructor.
    virtual ~CVFSEntry() {}

    //! \brief Returns a clone of this add-on.
  ...
  protected:
    ...
    bool m_livetv;            //!< VFS entry files supply a LiveTV interface.
  };
  ...
  //! \brief Wrapper equpping a CVFSEntry with an ILiveTV interface.
  //! \details Needed as VFSEntry implements several VFS interfaces
  //!          with overlapping methods.
  ...
  public:
    ...
    //! \brief Switch to the next channel.
    bool NextChannel(bool preview=false);

    //! \brief Switch to the previous channel.
    bool PrevChannel(bool preview=false);

    //! \brief Switch to a given channel.
    bool SelectChannel(unsigned int channel);

    //! \brief Get total playing time of channel.
    int GetTotalTime();

    //! \brief Get starting time for channel.
    int GetStartTime();
  ...
  //! \brief Manager for instances of a VFS add-on.
  //! \details Make sure only a single add-on is instanced and
  //           takes care of time unloads.
  ...
  public:
    ...
    //! \brief Obtain an instance of a VFS addon.
    //! \details Returns an already existing instance if found, else
    //!          creates a new instance.
    //! \param[in] id ID of requested VFS add-on.
    VFSEntryPtr GetAddon(const std::string& id);

    //! \brief Remove instances that has been idle for a while.
    void ClearOutIdle();
  ...
  protected:
    ...
    //! \details Private to enforce singleton usage.
    CVFSEntryManager();

    std::map<std::string, VFSEntryPtr> m_addons; //!< Maps from ID to instance.
  };
